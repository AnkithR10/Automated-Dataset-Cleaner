"use client";

import React from "react";
import { Loader2, X } from "lucide-react";

interface LoginCardProps {
  onGoogleLogin: () => Promise<void>;
  isLoading: boolean;
  onClose?: () => void;
  onDeveloperLogin?: (email: string) => Promise<void>;
}

export function LoginCard({ onGoogleLogin, isLoading, onClose }: LoginCardProps) {
  return (
    <div className="w-full max-w-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-md flex flex-col justify-between transition-all duration-300">
      
      {/* Top row with inline logo and close button */}
      <div className="flex items-center justify-between mb-5 border-b border-slate-100 dark:border-slate-800/80 pb-3">
        <div className="flex items-center gap-2">
          <div className="h-2.5 w-2.5 rounded-full bg-blue-600" />
          <span className="font-bold text-slate-900 dark:text-white text-xs tracking-tight">
            Dataset Cleaner
          </span>
        </div>
        {onClose && (
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 p-1 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors cursor-pointer"
            title="Go Back"
            disabled={isLoading}
          >
            <X size={14} />
          </button>
        )}
      </div>

      {/* Header Stack */}
      <div className="text-center mb-6">
        <h3 className="text-lg font-bold text-slate-900 dark:text-white tracking-tight">
          Welcome Back
        </h3>
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1.5 leading-relaxed">
          Authenticate securely using your Google Account to access your workspace.
        </p>
      </div>

      {/* Google Sign In */}
      <button
        onClick={onGoogleLogin}
        disabled={isLoading}
        className="w-full px-4 py-3 text-sm font-bold rounded-xl bg-white dark:bg-slate-900 border-2 border-slate-800 dark:border-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-800 dark:text-white transition-all flex items-center justify-center gap-2.5 shadow-md hover:shadow-lg cursor-pointer"
      >
        {isLoading ? (
          <>
            <Loader2 className="animate-spin" size={14} />
            Authenticating...
          </>
        ) : (
          <>
            <svg className="w-3.5 h-3.5 flex-shrink-0" viewBox="0 0 24 24">
              <path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
              <path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
              <path fill="currentColor" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z" />
              <path fill="currentColor" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z" />
            </svg>
            Sign In with Google
          </>
        )}
      </button>

    </div>
  );
}
